...
SELECT DISTINCT ?pw str(?pathway_name) as ?pw_name str(?pathway_category) as ?pw_category ?pathway_category_id str(?pathway_disease) as ?pw_disease (GROUP_CONCAT(DISTINCT ?enzymes_name ; separator = ",") AS ?enzymes_names) (GROUP_CONCAT(DISTINCT ?product_glycan_ids ; separator = ",") AS ?glycan_ids)
...
({
  json({list}){
    return list.results.bindings.map((row) => {
      ...
      return{
        "pathway_id": row.pw.value.replace("http://glycosmos.org/biopax/1/562#GC-Pathway-", ""),
